...
func logRegistryInfo(registry map[uint8]RelayedChain) {
	log.Debug().Msg("Current registry information:")
	for domainID, chain := range registry {
		log.Debug().Msgf("- DomainID: %v, Chain: %v", domainID, chain)
	}
}
